...
<form action="borrow2.php" method="post">
	...
	<tr>
<td></td><td>
	<input type="submit" value="借阅" name="borrowYes" class="red button"/> <input type="reset" class="grey button"/>
	</td>
	</tr>
		</table></form>
</br>
<?php
	...
	$row = mysql_fetch_array($result);
	if ($row["cardNum"]!=NULL){
		exit("<font color=0xffff00 size=3>您已借过此书!</font>");
	}
	$q = "SELECT amount FROM book WHERE bookNum='".$bookNum."'";
	$result = mysql_query($q, $con);
	$row = mysql_fetch_array($result);
	if ($row["amount"]==NULL){
		exit("<font color=0xffff00 size=3>该书不存在!</font>");
	}
	else if ($row["amount"]==0){
		exit("<font color=0xffff00 size=3>该书已被借完!</font>");
	}
	else{
		$q = "UPDATE book SET amount=amount-1 WHERE bookNum='".$bookNum."'";
		mysql_query($q, $con);	
	}
	$q = "INSERT INTO record VALUES('".$_COOKIE["cardID"]."', '".$bookNum."', '".$time."', NULL, '".$adminID."')";
	mysql_query($q, $con);
	exit("<font color=0xffff00 size=3>借阅成功!</font>");
?>
</center>